...
(function () {
  ...
  /* ================================================================
     Variant resolution
     keep in sync z SPECS: wireframes/src/content/variants.ts
     Logika 1:1 z resolveVariantFromUrl() we wszystkich HTML.
  ================================================================ */

  // Lista 9 dopuszczalnych id — te same co klucze VARIANTS w HTML
  var KNOWN_VARIANTS = {
    'gads-kary':      1,
    'gads-podlegam':  1,
    'gads-audyt':     1,
  ...
  // Mapa utm_campaign → variant id — te same co UTM_CAMPAIGN_MAP w HTML
  var CAMPAIGN_MAP = {
    'nis2-kary':      'gads-kary',
  ...
  // Fallback — musi być zgodny z wartością zwracaną przez resolveVariantFromUrl() w HTML
  var DEFAULT_VARIANT = 'gads-kary';

  function resolveVariant() {
    var explicit = getParam('variant');
    if (explicit && KNOWN_VARIANTS[explicit]) return explicit;
    var campaign = getParam('utm_campaign');
    if (campaign && CAMPAIGN_MAP[campaign]) return CAMPAIGN_MAP[campaign];
